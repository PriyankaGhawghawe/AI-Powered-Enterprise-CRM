...
def init_db_data():
    ...
    try:
        ...
        # Seed default business data if empty
        from app.models import BusinessMetric, HistoricalPerformance, SalesDeal, Competitor, RegulatoryRisk, ComplianceChecklist
        ...
        if db.query(BusinessMetric).first() is None:
            ...
            metrics_data = {
                ...
                "financials.cash_balance": "350000",
                "financials.monthly_mrr": "45000",
                "financials.expenses.payroll": "32000",
                "financials.expenses.marketing": "8000",
                "financials.expenses.hosting_cloud": "4000",
                "financials.expenses.software_licenses": "2000",
                "financials.expenses.office_rent": "1500",
                "financials.expenses.miscellaneous": "500",
            }
            for k, v in metrics_data.items():
                db.add(BusinessMetric(key=k, value=v))
            ...
            for h in history_data:
                db.add(HistoricalPerformance(month=h["month"], revenue=h["revenue"], expenses=h["expenses"]))

            # Seed Sales Deals
            deals_data = [
                {"id": "deal_1", "name": "Acme Corp Enterprise", "value": 25000, "stage": "Proposal", "probability": 60, "owner": "Sarah Jenkins", "age_days": 45},
                {"id": "deal_2", "name": "GlobalTech Integration", "value": 18000, "stage": "Negotiation", "probability": 80, "owner": "Sarah Jenkins", "age_days": 60},
                {"id": "deal_3", "name": "Nova Retail Systems", "value": 12000, "stage": "Demo", "probability": 40, "owner": "Mike Ross", "age_days": 12},
                {"id": "deal_4", "name": "Apex Consulting", "value": 8000, "stage": "Lead", "probability": 20, "owner": "Mike Ross", "age_days": 5},
                {"id": "deal_5", "name": "Vortex Finance", "value": 30000, "stage": "Negotiation", "probability": 90, "owner": "Sarah Jenkins", "age_days": 90},
                {"id": "deal_6", "name": "EcoSphere Logistics", "value": 15000, "stage": "Closed Won", "probability": 100, "owner": "Mike Ross", "age_days": 30},
                {"id": "deal_7", "name": "Stellar Media", "value": 10000, "stage": "Closed Lost", "probability": 0, "owner": "Mike Ross", "age_days": 40},
            ]
            for d in deals_data:
                db.add(SalesDeal(id=d["id"], name=d["name"], value=d["value"], stage=d["stage"], probability=d["probability"], owner=d["owner"], age_days=d["age_days"]))

            # Seed Competitors
            competitors_data = [
                {"name": "DocuGenie", "market_share": "32%", "pricing": "$49/user/month", "strengths": "Rich template library, established brand", "weaknesses": "Slow generation, outdated editor"},
                {"name": "WriteAI Pro", "market_share": "18%", "pricing": "$29/user/month", "strengths": "Fast generation speed, low pricing", "weaknesses": "Poor formatting, lacks API integrations"},
                {"name": "AutoDoc Co", "market_share": "12%", "pricing": "$99/user/month", "strengths": "Excellent API integrations, enterprise grade security", "weaknesses": "Complex onboarding, high pricing"},
            ]
            for c in competitors_data:
                db.add(Competitor(name=c["name"], market_share=c["market_share"], pricing=c["pricing"], strengths=c["strengths"], weaknesses=c["weaknesses"]))

            # Seed Regulatory Risks
            risks_data = [
                {"risk_area": "Data Privacy", "description": "New EU AI Act requirements demand transparency logs for generated content.", "severity": "Medium", "mitigation": "Incorporate audit trails into product backend by Q4 2026."}
            ]
            for r in risks_data:
                db.add(RegulatoryRisk(risk_area=r["risk_area"], description=r["description"], severity=r["severity"], mitigation=r["mitigation"]))

            # Seed Compliance Checklist
            checklist_data = [
                {"item": "Data Processing Agreements (DPA) signed with all sub-processors", "status": "Completed"},
                {"item": "User consent opt-in implemented on all lead capture forms", "status": "Completed"},
                {"item": "Data breach notification procedure established and documented", "status": "Completed"},
                {"item": "Annual employee privacy training completed", "status": "Pending (Due: Aug 2026)"},
                {"item": "CCPA compliance mapping completed", "status": "In Progress"},
            ]
            for cl in checklist_data:
                db.add(ComplianceChecklist(item=cl["item"], status=cl["status"]))
...
async def get_analytics():
    ...
    try:
        current_cash = int(metrics.get("financials.cash_balance", 350000))
        current_mrr = int(metrics.get("financials.monthly_mrr", 45000))
    except ValueError:
        current_cash = 350000
        current_mrr = 45000
