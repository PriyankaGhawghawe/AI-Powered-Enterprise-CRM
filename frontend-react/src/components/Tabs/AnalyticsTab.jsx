    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">AI-assisted SQL sandbox for data exploration.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Left: NL to SQL */}
        <div className="bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col">
          ...
          <textarea
            ...
            className="w-full flex-grow bg-slate-900/50 border border-white/10 rounded-xl p-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 min-h-[160px] resize-none mb-4 transition-all"
          />
          <button
            onClick={handleGenerate}
            disabled={loadingGenerate || !nlQuery.trim()}
            className="w-full flex justify-center items-center gap-2 bg-purple-500 hover:bg-purple-600 disabled:opacity-50 text-white font-medium py-3 px-4 rounded-xl transition-colors mt-auto"
          >
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-xl flex flex-col">
          ...
          <textarea
            ...
            className="flex-grow w-full bg-slate-900/80 border border-white/10 rounded-xl p-4 text-green-400 font-mono text-sm placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500/50 min-h-[160px] resize-none mb-4 transition-all shadow-inner"
          />
          <button
            onClick={handleExecute}
            disabled={loadingExecute || !sqlQuery.trim()}
            className="w-full flex justify-center items-center gap-2 bg-blue-500 hover:bg-blue-600 disabled:opacity-50 text-white font-medium py-3 px-4 rounded-xl transition-colors mt-auto"
          >
      ...
      <div className="bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-slate-800/80">
          <h3 className="text-lg font-semibold text-white flex items-center gap-2">
            <FaTable className="text-emerald-400" /> Results {rows.length > 0 && <span className="text-xs font-normal text-gray-400 bg-slate-700 px-2 py-1 rounded-full">{rows.length} rows</span>}
          </h3>
        </div>
        <div className="p-0 overflow-x-auto max-h-[500px]">
          {columns.length === 0 ? (
            ...
            <table className="w-full text-sm text-left text-gray-300 relative">
              <thead className="text-xs text-gray-400 uppercase bg-slate-900/80 sticky top-0 z-10 backdrop-blur-sm">
                <tr>
                  {columns.map((col, idx) => {
                      ...
                      return (
                        <td key={colIdx} className="px-6 py-4">
                          <div className="max-w-md overflow-hidden text-ellipsis whitespace-nowrap hover:whitespace-normal hover:break-words">
                            {isNull ? (
                              <span className="text-xs italic text-gray-500 bg-slate-900/50 px-2 py-1 rounded-md">NULL</span>
                            ) : typeof val === 'object' ? (
      ...
      {rows.length > 0 && columns.length > 0 && (
        <div className="bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-xl mt-6 p-6">
          {!showChart ? (
            <div className="flex flex-col items-center justify-center p-8 text-center">
              <h3 className="text-xl font-semibold text-white mb-2">Analyze Your Data</h3>
              ...
              <button
                ...
                className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium py-3 px-6 rounded-xl transition-colors shadow-lg shadow-indigo-500/20"
              >
                {generatingViz ? (
                  <>
            ...
            <>
              ...
              <div className="w-full h-[400px] flex items-center justify-center bg-slate-900/30 rounded-xl border border-white/5 p-4">
                {chartData ? (
                  chartType === 'Bar' ? (
                    <Bar
                      data={chartData}
                  ...
                  <p className="text-gray-500">Not enough data to render chart.</p>
                )}
              </div>
            </>
          )}
