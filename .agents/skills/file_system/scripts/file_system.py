...
def read_report(filename: str, tool_context: ToolContext) -> dict:
    ...
    base_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "../../app/reports"))
    ...
    if not file_path.startswith(base_dir):
        ...
        return {"status": "error", "message": "Access Denied: Path traversal detected outside sandboxed directory."}
...
def write_document(filename: str, content: str, tool_context: ToolContext) -> dict:
    ...
    # Sandbox Egress Guard: prevent directory traversal
    if not file_path.startswith(base_dir):
        log_action(role, f"Traversal block on write: {filename}", "FileSystem MCP", "Denied")
        return {"status": "error", "message": "Access Denied: Path traversal detected outside sandboxed directory."}
    ...
    if any(keyword in content for keyword in unsafe_keywords):
        log_action(role, f"Security Injection Block: {filename}", "FileSystem MCP", "Denied")
        return {"status": "error", "message": "Access Denied: Malicious dynamic code patterns or dynamic import executions detected in content."}
